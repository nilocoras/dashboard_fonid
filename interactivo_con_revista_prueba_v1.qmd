...
nodos$name <- c(
  ...
  paste("Cambia de Puesto en la Escuela X (", round(100 * sum(rotacion$estado_final12 == "cambio_puesto") / total, 1), "%)", sep = ""),
  paste("Asciende a directivo en la Escuela X (", round(100 * sum(rotacion$estado_final12 == "ascenso (dire_vice)") / total, 1), "%)", sep = ""),
  paste("Abandona Sist. Educativo (", round(100 * sum(rotacion$estado_final12 == "abandono sistema") / total, 1), "%)", sep = ""),
  paste("Deja la Escuela X por otra (", round(100 * sum(rotacion$estado_final12 == "otro (cambio o dejo la escuela)") / total, 1), "%)", sep = ""),
  paste("Mantiene su posicion en la Escuela X (", round(100 * sum(rotacion$estado_final12 == "sin cambio") / total, 1), "%)", sep = "")
)
...
```{r}
seqdplot(rotacion.seq , sortv = "from.start", with.legend = FALSE ,main = paste("Evolución Maestros Titulares dic/22-23"), cex.legend = 0.7)
invisible()
...
sankey <- sankeyNetwork(Links = links, Nodes = nodos, Source = "partida", Target = "objetivo", Value = "valores", NodeID = "name", units = "Docentes", fontSize = 12, nodeWidth = 40,colourScale = node_colors)

sankey_with_title <- htmlwidgets::prependContent(
  sankey,
  htmltools::tags$h5("Estado Maestro Titular Dic 22 vs Dic 23",style = "font-weight: bold;text-align: center;font-seize: 11;"))
...
reactive_df_sankey_rotacion <- reactive({
  ...
  nodos_fil$name <- c(
    "Maestro de Grado (Titular) en Escuela X",
    paste("Cambia de Puesto en la Escuela X (", round(100 * sum(filtered_data_rotacion()$estado_final12 == "cambio_puesto") / total_fil, 1), "%)", sep = ""),
    paste("Asciende a directivo en la Escuela X (", round(100 * sum(filtered_data_rotacion()$estado_final12 == "ascenso (dire_vice)") / total_fil, 1), "%)", sep = ""),
    paste("Abandona Sist. Educativo (", round(100 * sum(filtered_data_rotacion()$estado_final12 == "abandono sistema") / total_fil, 1), "%)", sep = ""),
    paste("Deja la Escuela X por otra (", round(100 * sum(filtered_data_rotacion()$estado_final12 == "otro (cambio o dejo la escuela)") / total_fil, 1), "%)", sep = ""),
    paste("Mantiene su posicion en la Escuela X (", round(100 * sum(filtered_data_rotacion()$estado_final12 == "sin cambio") / total_fil, 1), "%)", sep = "")
  )
